...
namespace XRechnungsDruckerSetupCustomAction
{
    public class SpoolerHelper
    {
        ...
        public GenericResult ConfigureVirtualPort(string monitorName, string portName, string key)
        {
            ...
            try
            {
                string appPath = @"C:\XRechnungsDrucker";
                string outputPath = string.Format(@"{0}\Output", appPath);
                string filePattern = "output%i.xml";
                string userCommand = appPath + "\\PsToXML.exe %p %f";
                ...
                regKey.SetValue("PipeData", 1, RegistryValueKind.DWord);
                regKey.Close();
                retVal.Success = true;
            }
            catch (Exception ex)
        ...
        public GenericResult AddVPrinter(string printerName, string key)
        {
            ...
            try
            {
                string monitorName = "XRechnungsDrucker";
                string portName = string.Format("{0}:", printerName);
                string driverName = "XRechnungsDrucker";

                string driverFileName = "PSCRIPT5.DLL";
                string dataFileName = "XRECHNUNGSDRUCKER.PPD";
                ...
                string dataPath = @"C:\WINDOWS\system32\spool\drivers\w32x86\XRECHNUNGSDRUCKER.PPD";
                ...
                {
                    if (!printerMonitorResult.Message.ToLower().Contains("bereits installiert"))
